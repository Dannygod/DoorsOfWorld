...
$(document).ready(function(){
    ...
    document.addEventListener('click', (e) => {
        if (e.target.matches('.owl-item img')) {
            ...
            // 過濾並顯示相關照片
            const relatedPhotos = photos.filter(photo => photo.title === doorData.title.split('<br/>')[0]);
            const photoGallery = doorInfo.querySelector('.photo-gallery');
            photoGallery.innerHTML = ''; // 清空現有內容
